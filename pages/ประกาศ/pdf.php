...
function OderPDF($mpdfConfig, $request)
{
    ...
    $announce = !empty($request['announce']) ? $request['announce'] : '';
    $subject = !empty($request['subject']) ? $request['subject'] : '';
    $since = !empty($request['since']) ? $request['since'] : '';
    $announcedate = !empty($request['announcedate']) ? $request['announcedate'] : '';
    ...
    if (!empty($description)) {
        ...
        for ($i = 0; $i < $page_count; $i++) {
            ...
            if ($i == 0 && (count($data_array) > 1)) {
                $html = OderPage(0, $announce, $since, $subject, $announcedate, $data_array[$i], $rank, $signature, $namesurname, $position, $other);
            } else if ($i == 0 && count($data_array) == 1) {
                $html = OderPage(2, $announce, $since, $subject, $announcedate, $data_array[$i], $rank, $signature, $namesurname, $position, $other);
            } else if (count($data_array) == $i + 1) {
                $html = OderPage(1, $announce, $since, $subject, $announcedate, $data_array[$i], $rank, $signature, $namesurname, $position, $other);
            } else {
                $html = OderPage(3, $announce, $since, $subject, $announcedate, $data_array[$i], $rank, $signature, $namesurname, $position, $other);
            }
            $mpdf->WriteHTML($html);
        }
    }
...
function OderHeaderTemplate($announce, $subject)
{
...
function OderFooterTemplate($since, $announcedate)
{
  if (!empty($since)) {
    return <<<EOD
...
function OderPage($type, $announce, $since, $subject, $announcedate, $description, $rank, $signature, $namesurname, $position, $other)
{
    ...
    $css = announceCss();
    if ($type === 0) {
        $header = OderHeaderTemplate($announce, $subject);
    }
    if ($type === 1) {
        $oderfooter = OderFooterTemplate($since, $announcedate);
