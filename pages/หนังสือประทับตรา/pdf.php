...
$request = $_POST;
sealPDF($mpdfConfig, $request);


function sealPDF($mpdfConfig, $request)
{
    ...
    if (!empty($description)) {
        ...
        for ($i = 0; $i < $page_count; $i++) {
            ...
            if ($i == 0 && (count($data_array) > 1)) {
                $html = sealPage(0, $at, $to, $agencies, $monthyear, $storyowner, $phone, $fax, $data_array[$i], $rank, $signature, $namesurname, $position, $other);
            } else if ($i == 0 && count($data_array) == 1) {
                $html = sealPage(2, $at, $to, $agencies, $monthyear, $storyowner, $phone, $fax, $data_array[$i], $rank, $signature, $namesurname, $position, $other);
            } else if (count($data_array) == $i + 1) {
                $html = sealPage(1, $at, $to, $agencies, $monthyear, $storyowner, $phone, $fax, $data_array[$i], $rank, $signature, $namesurname, $position, $other);
            } else {
                $html = sealPage(3, $at, $to, $agencies, $monthyear, $storyowner, $phone, $fax, $data_array[$i], $rank, $signature, $namesurname, $position, $other);
            }
            $mpdf->WriteHTML($html);
        }
    }
...
function sealHeaderTemplate($at, $to)
{
...
function sealFooterTemplate($agencies, $monthyear, $storyowner, $phone, $fax)
{
    if (!empty($agencies)) {
        return <<<EOD
...
function sealPage($type, $at, $to, $agencies, $monthyear, $storyowner, $phone, $fax, $description, $rank, $signature, $namesurname, $position, $other)
{
    $header = "";
    $footer = "";
    $sealfooter = "";
    ...
    $css = sealCss();
    if ($type === 0) {
        $header = sealHeaderTemplate($at, $to);
    }
    if ($type === 1) {
        $sealfooter = sealFooterTemplate($agencies, $monthyear, $storyowner, $phone, $fax);
